SweptArea_Con_in <- function(coords, scale_limits, legend_title, x_limits, y_limits, additional_shapefiles = NULL, additional_colors = NULL, additional_border_patterns = NULL, breaks, labels) {
  ...
  coords$gridID <- idx
  agg <- aggregate(coords$TR_AREA, by = list(idx), FUN = mean, na.rm = T)
  ...
  grd <- grd %>%
    mutate(TR_AREA = ifelse(data1 < 1, NA, TR_AREA))
  
  colSums(st_drop_geometry(grd[, c("TR_AREA")]), na.rm = T)
  
  
  grd <- grd %>%
    filter(!is.na(as.numeric(TR_AREA)))
  
  baseMap <- ggplot() + 
    geom_sf(data = grd, mapping = aes(fill = as.numeric(TR_AREA)), color = NA) +
    scale_fill_viridis(discrete = FALSE, na.value = "transparent", limits = scale_limits, name = legend_title,  breaks = breaks, # Specify the breaks
                       labels = labels) +
