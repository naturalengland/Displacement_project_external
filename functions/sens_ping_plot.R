createMap_sens <- function(data,Pressure,Title, additional_shapefiles = NULL, additional_colors = NULL,additional_border_patterns = NULL, x_limits = NULL, y_limits = NULL,Heading) {
  ...
  data <- data %>%
    mutate({{ Pressure }} := ifelse({{ Pressure }} %in% 1:8, {{ Pressure }}, 9))
  ...
  baseMap <- ggplot() + 
    ...
    geom_sf(data = data, aes(col = as.factor(Pressure)), size = 1)+
    scale_color_manual(name = Heading, values = c("9" = "#440154",
                                                 "8" = "#472d7b",
                                                 "7" = "#3b528b",
                                                 "6" = "#2c728e",
                                                 "5" = "#21918c",
    ...
    ggtitle(Title) #plotted pings based on habitat.
